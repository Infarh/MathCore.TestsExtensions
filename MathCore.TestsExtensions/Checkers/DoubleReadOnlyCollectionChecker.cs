...
public class DoubleReadOnlyCollectionChecker
{
    ...
    public sealed class EqualityCheckerWithAccuracy : IDisposable
    {
        ...
        internal EqualityCheckerWithAccuracy(IReadOnlyCollection<double> ActualValues, double[] ExpectedValues, bool Not = false)
        {
    ...
    private readonly IReadOnlyCollection<double> _ActualReadOnlyCollection;

    /// <summary>Инициализация нового объекта проверки коллекции вещественных чисел</summary>
    /// <param name="ActualReadOnlyCollection"></param>
    internal DoubleReadOnlyCollectionChecker(IReadOnlyCollection<double> ActualReadOnlyCollection) => _ActualReadOnlyCollection = ActualReadOnlyCollection;
    ...
    {
        Assert.That
           .Value(_ActualReadOnlyCollection.Count)
           .IsEqual(ExpectedValues.Length, $"Размер коллекции {_ActualReadOnlyCollection.Count} не совпадает с ожидаемым размером {ExpectedValues.Length}");

        return new(_ActualReadOnlyCollection, ExpectedValues);
    }
    ...
    public EqualityCheckerWithAccuracy ValuesAreNotEqualTo(params double[] ExpectedValues)
    {
        ...
        return new(_ActualReadOnlyCollection, ExpectedValues, Not: true);
    }
    ...
    public DoubleReadOnlyCollectionChecker ValuesAreEqual(params double[] ExpectedValues)
    {
    ...
    /// <param name="ExpectedValues">Ожидаемые значения коллекции</param>
    public DoubleReadOnlyCollectionChecker ValuesAreEqual(string Message, params double[] ExpectedValues)
    {
    ...
    /// <param name="Message">Сообщение, выводимое в случае неудачи</param>
    public DoubleReadOnlyCollectionChecker IsEqualTo(IReadOnlyCollection<double> ExpectedReadOnlyCollection, string? Message = null)
    {
        Assert.That
           .Value(_ActualReadOnlyCollection.Count)
           .IsEqual(ExpectedReadOnlyCollection.Count, $"Размер коллекции {_ActualReadOnlyCollection.Count} не совпадает с ожидаемым размером {ExpectedReadOnlyCollection.Count}");
        ...
        try
        {
            ...
            actual_collection_enumerator = _ActualReadOnlyCollection.GetEnumerator();
            ...
            throw new AssertFailedException(message)
               ...
               .AddData("Actual", _ActualReadOnlyCollection);
        }
        finally
        {
            expected_collection_enumerator?.Dispose();
    ...
    /// <param name="Accuracy">Точность сравнения</param>
    /// <param name="Message">Сообщение, выводимое в случае неудачи</param>
    public DoubleReadOnlyCollectionChecker IsEqualTo(IReadOnlyCollection<double> ExpectedReadOnlyCollection, double Accuracy, string? Message = null)
    {
        if (double.IsNaN(Accuracy))
            throw new ArgumentException("Значение точности не может быть равно NaN", nameof(Accuracy));

        Assert.That.Value(_ActualReadOnlyCollection.Count).IsEqual(ExpectedReadOnlyCollection.Count, "Размеры коллекций не совпадают");
    ...
    /// <param name="Message">Сообщение, выводимое в случае неудачи</param>
    public DoubleReadOnlyCollectionChecker IsEqualTo(IReadOnlyCollection<double> ExpectedReadOnlyCollection, IEqualityComparer<double> Comparer, string? Message = null)
    {
    ...
    /// <param name="Message">Сообщение, выводимое в случае неудачи</param>
    public DoubleReadOnlyCollectionChecker ElementsAreEqualTo(double ExpectedValue, string? Message = null)
    {
        var index = 0;
        var assert_fails = new List<FormattableString>();
        var min_delta = double.PositiveInfinity;
        var max_delta = double.NegativeInfinity;
        var max_error = double.NegativeInfinity;
        foreach (var actual_value in _ActualReadOnlyCollection)
        {
    ...
    /// <param name="Message">Сообщение, выводимое в случае неудачи</param>
    public DoubleReadOnlyCollectionChecker ElementsAreEqualTo(double ExpectedValue, double Accuracy, string? Message = null)
    {
        if (double.IsNaN(Accuracy))
            throw new ArgumentException("Значение точности не может быть равно NaN", nameof(Accuracy));
    ...
    /// <param name="Message">Сообщение, выводимое в случае неудачи</param>
    public DoubleReadOnlyCollectionChecker ElementsAreSatisfyCondition(ElementChecker Condition, string? Message = null)
    {
        var index = 0;
        var assert_fails = new List<FormattableString>();
        foreach (var actual_value in _ActualReadOnlyCollection)
    ...
    /// <param name="Condition">Условие проверки всех элементов</param>
    /// <param name="Message">Сообщение, выводимое в случае неудачи</param>
    public DoubleReadOnlyCollectionChecker ElementsAreSatisfyCondition(PositionElementChecker Condition, string? Message = null)
    {
        var index = 0;
        var assert_fails = new List<FormattableString>();
        foreach (var actual_value in _ActualReadOnlyCollection)
    ...
    public DoubleValueChecker Max() => Assert.That.Value(_ActualReadOnlyCollection.Max());

    /// <summary>Минимальное значение в коллекции</summary>
    /// <returns>Объект проверки вещественного значения</returns>
    public DoubleValueChecker Min() => Assert.That.Value(_ActualReadOnlyCollection.Min());
